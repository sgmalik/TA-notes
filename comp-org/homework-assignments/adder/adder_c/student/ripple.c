...
int main(int argc, char *argv[]) {
    if (argc == 4) {
        // Full adder mode (3 inputs)
        int a = atoi(argv[1]);
        int b = atoi(argv[2]);
        int c_in = atoi(argv[3]);

        SumCarry r = full_adder(a, b, c_in);

        // output exactly what the autograder expects:
        printf("%d %d\n", r.sum, r.carry);
        return 0;

    } else if (argc == 3) {
        // Ripple-carry adder mode (2 inputs)
        unsigned a = atoi(argv[1]);
        unsigned b = atoi(argv[2]);

        RCAResult R = ripple_carry_adder(a, b);

        // output in a predictable, parseable format
        printf("%d ", R.carry);
        for (int i = 0; i < 4; i++) {
            printf("%d", R.bits[i]);
        }
        ...
        // No args: run your built-in test suite like before
        // Half adder cases
        ...
        // Ripple carry cases
        assertion_RCA(0, 0, 0, 0, 0, 0, 0);
        assertion_RCA(1, 0, 0, 0, 0, 0, 1);
        assertion_RCA(0, 1, 0, 0, 0, 0, 1);
        assertion_RCA(1, 1, 0, 0, 0, 1, 0);
        assertion_RCA(2, 2, 0, 0, 1, 0, 0);
        assertion_RCA(4, 4, 0, 1, 0, 0, 0);
        assertion_RCA(3, 3, 0, 0, 1, 1, 0);
        assertion_RCA(5, 5, 0, 1, 0, 1, 0);
        assertion_RCA(7, 7, 0, 1, 1, 1, 0);
        assertion_RCA(8, 7, 0, 1, 1, 1, 1);
        assertion_RCA(10, 5, 0, 1, 1, 1, 1);
        assertion_RCA(6, 6, 0, 1, 1, 0, 0);
        assertion_RCA(15, 1, 1, 0, 0, 0, 0);
        assertion_RCA(14, 14, 1, 1, 1, 0, 0);
        assertion_RCA(7, 12, 1, 0, 0, 1, 1);
        assertion_RCA(9, 9, 1, 0, 0, 1, 0);
        assertion_RCA(15, 15, 1, 1, 1, 1, 0);
        ...
        fprintf(stderr, "  %s a b c_in   # full adder mode\n", argv[0]);
        fprintf(stderr, "  %s a b        # ripple-carry adder mode\n", argv[0]);
        fprintf(stderr, "  %s            # run built-in tests\n", argv[0]);
        return 1;
    }
}
